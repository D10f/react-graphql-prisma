...
const { users, tokens } = require('./fixtures/seedData');
...
test('Should login using an existing user\'s credentials', async () => {
  ...
  const result = await mutate(mutations.LOGIN_USER, {
    variables: { username, password: 'UserInWonderLand' }
  });
...
test('Should NOT login using a invalid credentials', async () => {
  ...
  // This is an argon2 error because the user[2] password is not hashed in the mock database
...
test('Should retrieve a user\'s notifications', async () => {

  const result = await query(queries.GET_NOTIFICATIONS, {
    variables: { id: users[1].id }
  });

  // empty for now
  expect(result.data.getUserNotifications).toEqual([]);
});

test('Should NOT retrieve another user\'s notifications', async () => {
...
test('Should update an existing user\s data', async () => {
  ...
  const result = await mutate(mutations.UPDATE_USER, {
    variables: { id: users[1].id, input: updates }
  });
...
test('Should delete logged in user', async () => {
  ...
  const result2 = await query(queries.GET_USERS, {
    variables: { query: users[1].username }
  });
